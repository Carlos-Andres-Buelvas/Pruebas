...
direcciones = [f"Calle {i} #{random.randint(1, 99)}-{random.randint(1, 99)}" for i in range(1, 301)]
nombres_alojamientos = [f"Alojamiento{i:03d}" for i in range(1, 301)]
anotaciones = [
    "Sin comentarios", "Llegare tarde", "¿Tiene aire acondicionado?",
...
for i in range(1, 101):
    ...
    nombre = nombres_genericos[i + 500]
...
for docAnfitrion in docs_anfitriones:
    ...
    for _ in range(cantidad):
        if contador_alojamientos > 300:
            break
        codigo = f"ALOJ{contador_alojamientos:03d}"
...
while len(reservas) < 251:
    ...
    fechaPago = (entrada - timedelta(days=random.randint(1, 8))).strftime("%d/%m/%Y")
...
print("Archivos generados correctamente con 70 anfitriones, 100 huéspedes y 250 reservas.")
